...
This is the operational runbook for the CurveLens WTI deployment. The agent
must also follow the repository-level `AGENTS.md` framework and safety rules.
...
Before any runtime command, set and verify:

```bash
export CCVM_PRODUCT=wti
```

Runtime state automatically resolves to `ccvm/data/products/wti/`. Run commands from the
...
(`curvelens-core/`); the Python interpreter is the pipeline's venv at `ccvm/.venv`.
...
1. Download the CME Section 63 Energy Options bulletin with the existing headed
   ...
   `CME_BULLETIN_RUN_PIPELINE=0 CME_BULLETIN_DIR=ccvm/data/products/wti/cme_bulletin /Users/openclawyi/.openclaw/workspace/skills/cme-bulletin-downloader/scripts/download_cme_bulletin_pdf.js`.
...
4. After the PDF has been saved to `ccvm/data/products/wti/cme_bulletin/<date>.pdf`, run
...
7. Run `ccvm/.venv/bin/python agent/notify.py --prepare --date <date>`, where
   ...
   `PRIORITY_ALERT`, queueing them in `ccvm/data/products/wti/agent_outbox/pending.json`. It skips
...
same day — no bulletin needed (see `deployments/wti/cron.example`; schedule sourced
...
Runtime state (under `ccvm/data/products/wti/`, gitignored):
- `ccvm/data/products/wti/cme_bulletin/<date>.pdf` — the agent-downloaded CME bulletin
- `ccvm/data/products/wti/reports/<date>.{md,json}` — the daily brief
- `ccvm/data/products/wti/agent_outbox/pending.json` — messages awaiting Telegram delivery + ack
- `ccvm/data/products/wti/agent_outbox/delivered.json` — delivery log (dedupe guarantee)
...
- The agent must not send Telegram messages except for pending `DAILY_BRIEF` /
  `PRIORITY_ALERT` items in `ccvm/data/products/wti/agent_outbox/pending.json`, or an explicit
  approved test.
