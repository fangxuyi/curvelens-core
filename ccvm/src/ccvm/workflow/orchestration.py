...
def _write_synthesis_task(state: dict[str, Any]) -> None:
    ...
    task_path.write_text(
        ...
        "covered; set forecast_ledger to an empty list for a blocked synthesis. Leave memory_feedback empty "
        "when the manifest supplies no learning advisories. Active advisories are optional hypotheses, not "
        "evidence. Shadow advisories must not influence any view, confidence, or wording; record only whether "
        "each would have been used through its shadow disposition. "
