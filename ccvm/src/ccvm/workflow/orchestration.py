...
def _write_research_plan_task(state: dict[str, Any]) -> None:
    ...
    investigation_id_rule = manifest["research_contract"]["investigation_id_rule"]
    ...
    task_path.write_text(
        ...
        f"{investigation_id_rule} {citation_rule} "
...
def _write_role_tasks(state: dict[str, Any], repo_root: Path) -> None:
    ...
    for role, investigation in investigations.items():
        ...
        task_path.write_text(
            ...
            "confirmations, invalidations, and any unresolved question. Keep "
            "evidence_ids and counterevidence_ids disjoint: no ID may appear in both lists. "
            "Set counterevidence_ids to [] when no distinct contrary evidence exists. Every finding's "
...
def _write_synthesis_task(state: dict[str, Any]) -> None:
    ...
    task_path.write_text(
        ...
        "dispatched investigation in investigator_feedback as used, partially_used, or rejected. Each feedback "
        "object must contain exactly investigation_id, disposition, rationale, used_finding_ids, and evidence_ids; "
        "used_finding_ids and evidence_ids are always lists of strings. Use the field name used_finding_ids, never "
        "finding_ids. Return exactly one feedback object per dispatched investigation, or an empty list when none "
        "were dispatched. Copy the complete object shape from all three top_views in the template; do not rename or "
        "omit fields. Every top_views[*].key_metrics entry and every market_snapshot entry must be an object with "
        "label, value, comparison, plain_english_meaning, and evidence_ids. Every top_views key_metrics "
        "value must contain at least one numeric character and an evidence-backed measure. Never put "
        "categorical statuses such as confirmed, unavailable, or non_directional_uncertainty in value; "
        "keep them in view, story, limitation, or supporting-text fields and choose another cited numeric "
        "measure when a status is material. Every market_snapshot value must contain at least one numeric "
        "character and an evidence-backed measure. Keep categorical diagnostics such as invalid_surface or "
        "futures_only_repricing in story_chain or data_limitations and select another cited numeric "
        "observation for the snapshot. There is no top-level key_metrics field. "
        "For each top view, return exactly 2 or 3 key_metrics objects: the template's two are the default "
        "and a third is optional only when independently material. Never emit four or more and never silently "
        "truncate. "
        "Before validation, set synthesis.evidence_ids to the sorted, deduplicated union of every "
        ...
        "state the best-supported narrative change or explicitly say unexplained/not_applicable; "
        "option_market_readthrough must state whether options confirmed, faded, conflicted, were mixed, unavailable, "
        "or not material; forward_watch must state the next level, event, or release that confirms or invalidates "
        "the story. Include specific what_to_watch confirmations or invalidations. Build six to ten market_snapshot metric objects "
        "from exact values in canonical evidence, optionally using validated investigator key_metrics. The "
        "plain_english_summary must use short, "
